...
Why do natural languages look they way they do? While Zipf (1935) presented his “Principle of Least effort” as a domain general framework for understanding human behavior, he took language as his central case-study. He proposed that distributional properties found in natural language were evidence of speaker-listener utility maximization or, equivalently, effort minimization. In his own words, “we are arguing that people do in fact act with a maximum economy of effort, and that therefore in the process of speaking-listening they will automatically minimize the expenditure of effort.” Evidence supporting this hypothesis has been realized in a set of well-known findings, largely falling under what is now known as Zipf’s Law or Zipfian distributions -- one of a family of discrete power law distributions. A range of empirical phenomena in social and physical sciences are well-described as Zipfian, from the relationship between word rank and frequency (CITATION) to the populations of U.S. cities (CITATION) Stanford stats class 306a).\par

Language-structure and use provide a particularly fascinating case-study under an efficiency-based analysis as speaker- and listener-effort is asymmetric. Simply put, in the act of communicating with another person, what is effortful as a speaker is likely different from what is effortful as a listener. For example, Zipf noted that purely from the standpoint of speaker effort, what Zipf called "Speaker's Economy," an optimal language $\ell_{speaker}^*$ would tend toward a vocabulary of just a single, low-cost word. Given such a language, the full set of potential meanings would be conveyed using only that word, i.e. $\ell_{speaker}^*$ would be fully ambiguous and all possible meanings would need to be disambiguated by a listener. Conversely, from the standpoint of listener effort, what Zipf called "Auditor's Economy," an optimal language $\ell_{listener}^*$ would bijectively map all possible meanings to distinct words, eliminating a listener's need to disambiguate. Under this analysis, speaker effort is related to production cost and listener effort to understanding (disambiguation) cost. Clearly natural languages fall between the two extremes of $\ell_{speaker}^*$ and $\ell_{listener}^*$. Zipf’s Law of Meaning proposes that the particular relation between a word’s frequency and denotation size emerge from the competition of these forces -- the pressure to jointly minimize speaker and listener effort.\par

A range of more recent approaches to language science have distinctly Zipfian roots (functionalist theories). In terms of our opening question, “why do languages look the way they do?”, a functionalist approach would formalize the question in terms of information-theoretic principles of efficiency -- how does having property $x$ make using language $\ell$ more or less useful for communication? This family of theories has described a range of phenomena in language-design (compositionality, ambiguity, dependency length) and language-use (redundancy and reduction) in terms of communicative efficiency. Of particular interest to our current work, Piantadosi et al. (2012) argued that efficiently *designed* linguistic systems will contain ambiguous lexical items when communication is contextualized. If words with multiple meanings can be disambiguated in context, less costly forms can be re-used efficiently. They showed in a series of analyses that … [WHAT PIANTADOSI DID HERE!!!!]. \par

Zipf’s principle of least effort and it’s associated speaker and listener instantations were intended as a framework for explaining all of natural language. Evidence for the theory was largely derived from vocabulary-level analysis. Implicit in Zipf’s (1935) argument is that aggregate, distributional-level effects emerge from optimization during local interactions between speaker-listeners. While Functionalist projects have provided evidence that speaker’s attempt to optimize their output at a variety of levels -- phonologically (morphological contraction; Bell et al., 2003), syntactically (Levy & Jaeger, 2007; Jaeger, 2010) and over even over discourse (Genzel & Charniak, 2002), Zipf was less precise in terms of his exact level of analysis -- however his example of optimal speaker and listener language in which a single word $u$ can express some meaning $m$$[[u]]\neq \emptyset$.
...
In this work we present a step toward formalizing this connection highlighted by Horn -- formalizing the theoretical connection between Zipfian and Grice, and in doing so, deriving aggregate, distributional level properties from local, in-the-moment interactions of speaker-listeners. To gain traction on this project we adopt a simulation-based approach, modeling communication as reference games in which we can vary the types of agents and languages present. This framework allows us to investigate a range of functionalist theories broadly under the categories of efficient language design and language use. \par

For this particular project we focus on an analysis of the communicative function of ambiguity in language following work by Piantadosi et al. (2012). We adopt this particular linguistic phenomena to demonstrate the extent to which we can explore both the design- and use- based functionalist ideas. We focus on three questions -- (1) when is it desirable to have a language with lexical ambiguity? (2) Under what circumstances do speakers and listeners use ambiguity efficiently?\par

We begin with a high-level introduction to the modelling framework introducing the basic ingredients we will need to represent language as repeated reference games. Following this introduction we derive a simple objective function for measuring the efficiency of linguistics systems in this setting. Subsequently, we move on to two case-studies examining the questions posed above, framing results in terms of our efficiency measure.
