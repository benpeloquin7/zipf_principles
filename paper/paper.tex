...
Speakers and listeners can induce joint distributions over
utterance-meaning pairs, although, these distributions may differ:
...
context\footnote{In the current set of simulations we consider utterance costs as independent from context (i.e.. $p(u|c)p(c)=p(u)p(c)$).}
...
according to the following generative model -- some context occurs in
...
use \(\ell_1\) (because speaking it is always lower cost), it is more
difficult for a listener (because \(u_1\) is ambiguous). Crucially, if
context is disambiguating then the speaker can use \(u_1\) to refer to
either \(m_1\) or \(m_2\) and \(\ell_1\) should be preferred to
\(\ell_2\).\par
...
\fbox{\parbox[b][][c]{7.3cm}{\centering {SI and simulations: \href{https://bit.ly/2RBSGcU}{\url{https://bit.ly/2RBSGcU}}, \href{https://github.com/benpeloquin7/zipf_principles}{\url{https://github.com/benpeloquin7/zipf_principles}}}}}
\vspace{1em} \noindent
